class BaseDialog extends BaseView {
    ...
    getButtons() {
        return this.__bd_buttons.map(btn => `<a id="${btn.id || ''}" class="waves-effect waves-light ${btn.close === false ? '' : 'modal-close'} waves-effect waves-${btn.color} btn${btn.filled ? '' : '-flat'}">${btn.text}</a>`).join('\n')
    }
